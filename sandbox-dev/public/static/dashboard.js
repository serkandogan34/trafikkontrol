...
async function handleDomainSubmit(e) {
    ...
    const originalBackend = document.getElementById('originalBackend').value.trim();
    ...
    if (!originalBackend) {
        showNotification('Original Backend IP gerekli', 'error');
        return;
    }
...
// Duplicate removed - checkDNSPropagation exists at line 6724
...
// Duplicate removed - loadTrafficData exists at line 895
...
// Load DNS Records
// Duplicate removed - loadDNSRecords exists at line 387
...
// =============================================================================
// HOSTINGER DNS MANAGEMENT INTEGRATION
// =============================================================================

// Hostinger API configuration
let hostingerApiKey = localStorage.getItem('hostinger_api_key') || '';

// Show Hostinger DNS configuration modal
function showHostingerConfig() {
    const modalHTML = `
        <div id="hostingerConfigModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gray-800 p-6 rounded-lg w-full max-w-2xl">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-bold text-white flex items-center">
                        <i class="fas fa-server mr-3 text-orange-400"></i>
                        🏠 Hostinger DNS Configuration
                    </h3>
                    <button onclick="closeHostingerConfig()" class="text-gray-400 hover:text-white">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                ...
                <div class="space-y-6">
                    <!-- hPanel Information -->
                    <div class="bg-yellow-700 p-4 rounded-lg border border-yellow-600">
                        <h4 class="text-lg font-semibold text-white mb-3 flex items-center">
                            <i class="fas fa-exclamation-triangle mr-2 text-yellow-400"></i>
                            Hostinger hPanel Bilgilendirmesi
                        </h4>
                        <div class="text-sm text-yellow-100 space-y-2">
                            <p><strong>Önemli:</strong> Hostinger Business Web Hosting (hPanel) planlarında API desteği bulunmamaktadır.</p>
                            <p>DNS yönlendirmeleri <strong>manuel olarak</strong> yapılmalıdır.</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-700 p-4 rounded-lg">
                        <h4 class="text-lg font-semibold text-white mb-4">
                            ...
                            Manuel DNS Yönlendirme Rehberi
                        </h4>
                        
                        <div class="space-y-4 text-sm">
                            <div class="bg-gray-600 p-3 rounded border-l-4 border-blue-400">
                                <h5 class="font-semibold text-blue-300 mb-2">1. Hostinger hPanel'e Giriş</h5>
                                <ul class="text-gray-300 space-y-1 ml-4">
                                    <li>• Hostinger hesabınıza giriş yapın</li>
                                    <li>• <strong>Hosting</strong> → <strong>Manage</strong> bölümüne gidin</li>
                                    <li>• <strong>DNS/Nameservers</strong> sekmesini seçin</li>
                                </ul>
                            </div>

                            <div class="bg-gray-600 p-3 rounded border-l-4 border-green-400">
                                <h5 class="font-semibold text-green-300 mb-2">2. A Record Ekleme</h5>
                                <div class="bg-gray-800 p-2 rounded font-mono text-xs mb-2">
                                    <div class="text-green-400">Type: A</div>
                                    <div class="text-yellow-400">Name: @</div>
                                    <div class="text-blue-400">Value: [SERVER_IP_ADDRESS]</div>
                                    <div class="text-purple-400">TTL: 3600</div>
                                </div>
                                <p class="text-gray-300">Ana domain'i server'ınıza yönlendirir (örn: feroxilprostatit.online)</p>
                            </div>

                            <div class="bg-gray-600 p-3 rounded border-l-4 border-purple-400">
                                <h5 class="font-semibold text-purple-300 mb-2">3. CNAME Record (Opsiyonel)</h5>
                                <div class="bg-gray-800 p-2 rounded font-mono text-xs mb-2">
                                    <div class="text-green-400">Type: CNAME</div>
                                    <div class="text-yellow-400">Name: www</div>
                                    <div class="text-blue-400">Value: feroxilprostatit.online</div>
                                    <div class="text-purple-400">TTL: 3600</div>
                                </div>
                                <p class="text-gray-300">www alt domain'ini ana domain'e yönlendirir</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="text" id="manualDomainName" 
                                       placeholder="feroxilprostatit.online"
                                       class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="manualBackendIP" 
                                       placeholder="127.0.0.1:8080"
                                       class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            </div>
                            
                            <button onclick="generateNginxConfig()" 
                                    class="w-full bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-white font-medium">
                                <i class="fas fa-file-code mr-2"></i>Nginx Config Oluştur
                            </button>
                        </div>
                    </div>
                    
                    <!-- Generated Configuration Display -->
                    <div id="generatedNginxConfig" class="hidden bg-gray-700 p-4 rounded-lg">
                        <h4 class="text-lg font-semibold text-white mb-4">
                            <i class="fas fa-file-code mr-2 text-purple-400"></i>
                            Oluşturulan Nginx Configuration
                        </h4>
                        <div class="bg-gray-900 p-4 rounded overflow-x-auto">
                            <pre id="nginxConfigCode" class="text-green-400 text-sm font-mono"></pre>
                        </div>
                        <div class="mt-3 flex space-x-3">
                            <button onclick="copyNginxConfig()" 
                                    class="bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded-lg text-white font-medium">
                                <i class="fas fa-copy mr-2"></i>Kopyala
                            </button>
                            <button onclick="downloadNginxConfig()" 
                    ...
                    <div class="bg-gray-700 p-4 rounded-lg">
                        <h4 class="text-lg font-semibold text-white mb-4">
                            <i class="fas fa-cloud mr-2 text-cyan-400"></i>
                            Alternatif Ücretsiz DNS Servisleri
                        </h4>
                        
                        <div class="space-y-3">
                            <div class="flex items-center justify-between bg-gray-600 p-3 rounded">
                                <div class="flex items-center">
                                    <i class="fab fa-cloudflare text-orange-400 mr-3"></i>
                                    <div>
                                        <h5 class="text-white font-medium">Cloudflare DNS</h5>
                                        <p class="text-gray-300 text-sm">Ücretsiz + API destekli</p>
                                    </div>
                                </div>
                                <button onclick="showCloudflareSetup()" 
                                        class="bg-orange-600 hover:bg-orange-700 px-3 py-1 rounded text-white text-sm">
                                    Kurulum
                                </button>
                            </div>
                            ...
                            <div class="flex items-center justify-between bg-gray-600 p-3 rounded">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <h5 class="text-white font-medium">FreeDNS</h5>
                                        <p class="text-gray-300 text-sm">Ücretsiz dinamik DNS</p>
                                    </div>
                                </div>
                                <button onclick="window.open('https://freedns.afraid.org', '_blank')" 
                    ...
                    <div class="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <button onclick="exportDNSSettings()" 
                                    class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-white font-medium text-sm">
                                <i class="fas fa-file-export mr-2"></i>DNS Export
                            </button>
                            <button onclick="generateSSLConfig()" 
                                    class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-white font-medium text-sm">
                                <i class="fas fa-lock mr-2"></i>SSL Config
                            </button>
                        </div>
                    </div>
                </div>
...
// Close Hostinger configuration modal
function closeHostingerConfig() {
    const modal = document.getElementById('hostingerConfigModal');
    if (modal) modal.remove();
}

// Generate Nginx Configuration for manual setup
function generateNginxConfig() {
    const domain = document.getElementById('manualDomainName').value;
    const backendIP = document.getElementById('manualBackendIP').value;
    
    if (!domain || !backendIP) {
        showNotification('Lütfen domain adı ve backend IP adresini girin', 'error');
        return;
    }
    
    const nginxConfig = `# Nginx Configuration for ${domain}
# Generated by Traffic Management Platform

server {
    listen 80;
    server_name ${domain} www.${domain};
    
    # Traffic Analysis & Bot Detection
    location / {
        # Real IP forwarding
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        ...
        proxy_pass http://YOUR_TRAFFIC_MANAGER_IP:3000/proxy-handler;
        proxy_redirect off;
...
server {
    ...
    document.getElementById('nginxConfigCode').textContent = nginxConfig;
    document.getElementById('generatedNginxConfig').classList.remove('hidden');
    
    showNotification('Nginx configuration generated successfully!', 'success');
}

// Copy nginx config to clipboard
function copyNginxConfig() {
    const configText = document.getElementById('nginxConfigCode').textContent;
    
    navigator.clipboard.writeText(configText).then(() => {
        showNotification('Nginx configuration copied to clipboard!', 'success');
    }).catch(err => {
...
// Download nginx config as file
function downloadNginxConfig() {
    const configText = document.getElementById('nginxConfigCode').textContent;
    const domain = document.getElementById('manualDomainName').value || 'domain';
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${domain}-nginx.conf`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
    
    showNotification('Nginx configuration downloaded!', 'success');
}

// Show Cloudflare DNS setup
function showCloudflareSetup() {
    const setupHTML = `
        <div id="cloudflareSetupModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gray-800 p-6 rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-bold text-white flex items-center">
                        <i class="fab fa-cloudflare mr-3 text-orange-400"></i>
                        Cloudflare DNS Setup
                    </h3>
                    <button onclick="document.getElementById('cloudflareSetupModal').remove()" 
                ...
                <div class="space-y-6 text-sm">
                    <div class="bg-orange-700 p-4 rounded border border-orange-600">
                        <h4 class="font-bold text-white mb-2">🚀 Cloudflare Avantajları</h4>
                        <ul class="text-orange-100 space-y-1">
                            <li>• Ücretsiz DNS yönetimi</li>
                            <li>• API desteği (otomasyon)</li>
                            <li>• DDoS koruması</li>
                            <li>• CDN hızlandırma</li>
                    ...
                    <div class="space-y-4">
                        <div class="bg-gray-700 p-4 rounded">
                            <h5 class="font-bold text-blue-300 mb-2">1. Cloudflare'e Kayıt Olun</h5>
                            <p class="text-gray-300">
                                <a href="https://cloudflare.com" target="_blank" class="text-blue-400 hover:underline">
                                    cloudflare.com
                                </a> adresinden ücretsiz hesap oluşturun
                            </p>
                        </div>
                        
                        <div class="bg-gray-700 p-4 rounded">
                            <h5 class="font-bold text-green-300 mb-2">2. Domain Ekleyin</h5>
                            <p class="text-gray-300">Cloudflare dashboard'da "Add a Site" ile domain'inizi ekleyin</p>
                        </div>
                        
                        <div class="bg-gray-700 p-4 rounded">
                            <h5 class="font-bold text-purple-300 mb-2">3. Nameserver Değişimi</h5>
                            <p class="text-gray-300 mb-2">Hostinger'da nameserver'ları değiştirin:</p>
                            <div class="bg-gray-800 p-2 rounded font-mono text-xs text-green-400">
                        ...
                        <div class="bg-gray-700 p-4 rounded">
                            <h5 class="font-bold text-yellow-300 mb-2">4. DNS Kayıtlarını Ekleyin</h5>
                            <div class="bg-gray-800 p-3 rounded">
                                <div class="text-xs font-mono space-y-1">
                                    <div><span class="text-green-400">Type:</span> A</div>
                                    <div><span class="text-yellow-400">Name:</span> @</div>
                                    <div><span class="text-blue-400">Value:</span> [SERVER_IP]</div>
                                    <div><span class="text-purple-400">Proxy:</span> Orange Cloud ON</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
// Export DNS settings
function exportDNSSettings() {
    const domain = document.getElementById('manualDomainName').value || 'example.com';
    const backendIP = document.getElementById('manualBackendIP').value || '127.0.0.1:8080';
    ...
    const dnsSettings = {
        ...
        instructions: [
            '1. Hostinger hPanel → DNS/Nameservers bölümüne gidin',
            '2. Yukarıdaki kayıtları manuel olarak ekleyin',
            '3. DNS propagation için 24 saat bekleyin',
            '4. nslookup ile test edin'
        ]
    };
    
    const blob = new Blob([JSON.stringify(dnsSettings, null, 2)], { type: 'application/json' });
    const url = window.URL.createObjectURL(blob);
    
    const a = document.createElement('a');
    a.href = url;
    a.download = `${domain}-dns-settings.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
    
    showNotification('DNS settings exported successfully!', 'success');
}
...
function generateSSLConfig() {
    ...
    if (!domain) {
        showNotification('Lütfen domain adını girin', 'error');
        return;
    }
    
    const sslInstructions = `# SSL Certificate Installation Guide for ${domain}
...
## Manual SSL (Kendi sertifikanız varsa)
...
`;
    ...
    const blob = new Blob([sslInstructions], { type: 'text/plain' });
    const url = window.URL.createObjectURL(blob);
    
    const a = document.createElement('a');
    a.href = url;
    a.download = `${domain}-ssl-setup.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
    
    showNotification('SSL setup guide downloaded!', 'success');
}

// Test Hostinger API connection
async function testHostingerConnection() {
    const apiKey = document.getElementById('hostingerApiKey').value;
    
    if (!apiKey) {
        showNotification('Please enter your Hostinger API key first', 'error');
        return;
    }
    
    try {
        showNotification('Testing Hostinger API connection...', 'info');
        
        const response = await fetch('/api/hostinger/test-connection', {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
        ...
        const statusEl = document.getElementById('hostingerConnectionStatus');
            ...
            statusEl.innerHTML = `
                <div class="bg-green-900 border border-green-600 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h5 class="text-green-300 font-bold">Connection Successful!</h5>
                            <p class="text-green-200 text-sm">${data.message}</p>
                            <p class="text-green-200 text-sm">Found ${data.domainCount} domain(s)</p>
                        </div>
                    </div>
                </div>
            `;
            showNotification('✅ Hostinger API connection successful!', 'success');
        } else {
            statusEl.innerHTML = `
                <div class="bg-red-900 border border-red-600 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h5 class="text-red-300 font-bold">Connection Failed</h5>
                            <p class="text-red-200 text-sm">${data.message}</p>
                        </div>
                    </div>
                </div>
            `;
            showNotification('❌ Hostinger API connection failed: ' + data.message, 'error');
        }
    } catch (error) {
        showNotification('Error testing Hostinger connection: ' + error.message, 'error');
    }
}

// Save Hostinger configuration
function saveHostingerConfig() {
    const apiKey = document.getElementById('hostingerApiKey').value;
    ...
    localStorage.setItem('hostinger_api_key', apiKey);
    hostingerApiKey = apiKey;
    
    showNotification('✅ Hostinger configuration saved', 'success');
    
    // Close modal after short delay
    setTimeout(() => {
        closeHostingerConfig();
    }, 1500);
}

// Load Hostinger domains
async function loadHostingerDomains() {
    const apiKey = hostingerApiKey || document.getElementById('hostingerApiKey').value;
    
    if (!apiKey) {
        showNotification('Please configure Hostinger API key first', 'error');
        return;
    }
    
    try {
        showNotification('Loading Hostinger domains...', 'info');
        ...
        const domainsList = document.getElementById('hostingerDomainsList');
        ...
        if (data.success && data.domains.length > 0) {
            domainsList.innerHTML = data.domains.map(domain => `
                <div class="bg-gray-600 p-3 rounded-lg flex justify-between items-center">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <div class="text-white font-medium">${domain.domain}</div>
                            <div class="text-gray-300 text-sm">Status: ${domain.status}</div>
                        </div>
                    </div>
                    <div class="flex space-x-2">
                        <button onclick="manageDomainDNS('${domain.domain}')" 
                                class="bg-purple-600 hover:bg-purple-700 px-3 py-1 rounded text-sm">
                            <i class="fas fa-cogs mr-1"></i>Manage DNS
                        </button>
                        <button onclick="syncDomainDNS('${domain.domain}')" 
            ...
            showNotification(`✅ Loaded ${data.domains.length} Hostinger domains`, 'success');
        } else if (data.success && data.domains.length === 0) {
            domainsList.innerHTML = `
                <div class="text-center text-gray-400 py-4">
                    <i class="fas fa-inbox text-2xl mb-2"></i>
                    <p>No domains found in your Hostinger account</p>
                </div>
            `;
        } else {
            showNotification('Failed to load Hostinger domains: ' + data.error, 'error');
        }
    } catch (error) {
        showNotification('Error loading Hostinger domains: ' + error.message, 'error');
    }
}

// Manage DNS for a specific Hostinger domain
async function manageDomainDNS(domain) {
    const apiKey = hostingerApiKey || localStorage.getItem('hostinger_api_key');
    ...
    try {
        showNotification(`Loading DNS records for ${domain}...`, 'info');
        
        const response = await fetch(`/api/hostinger/dns-records/${domain}`, {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
        ...
        if (data.success) {
            showHostingerDNSManager(domain, data.records);
        } else {
            showNotification('Failed to load DNS records: ' + data.error, 'error');
        }
    } catch (error) {
...
// Show Hostinger DNS manager for a specific domain
function showHostingerDNSManager(domain, records) {
    const modalHTML = `
        <div id="hostingerDNSModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gray-800 p-6 rounded-lg w-full max-w-6xl max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-bold text-white flex items-center">
                        <i class="fas fa-dns mr-3 text-purple-400"></i>
                        Hostinger DNS Manager - ${domain}
                    </h3>
                    <button onclick="closeHostingerDNSManager()" class="text-gray-400 hover:text-white">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                            <div>
                                ...
                                <select id="newHostingerRecordType" class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            ...
                            <div>
                                ...
                                <input type="text" id="newHostingerRecordName" placeholder="@, www, mail..."
                                       class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">Content</label>
                                <input type="text" id="newHostingerRecordContent" placeholder="IP address, domain..."
                                       class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">TTL</label>
                                <select id="newHostingerRecordTTL" class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            ...
                            <div class="flex items-end">
                                <button onclick="addHostingerDNSRecord('${domain}')" 
                                        class="w-full bg-green-600 hover:bg-green-700 px-4 py-2 rounded-lg text-white font-medium">
                    ...
                    <div class="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div id="hostingerDNSRecordsTable">
                            ${generateHostingerDNSTable(records)}
                        </div>
                    </div>
                </div>
            </div>
...
// Generate Hostinger DNS records table
function generateHostingerDNSTable(records) {
    ...
    return `
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-600">
                        <th class="text-left py-2 px-3 text-gray-300">Type</th>
                        <th class="text-left py-2 px-3 text-gray-300">Name</th>
                        <th class="text-left py-2 px-3 text-gray-300">Content</th>
                        <th class="text-left py-2 px-3 text-gray-300">TTL</th>
                        <th class="text-left py-2 px-3 text-gray-300">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    ${records.map(record => `
                        <tr class="border-b border-gray-700 hover:bg-gray-600">
                            <td class="py-2 px-3 text-white">${record.type}</td>
                            <td class="py-2 px-3 text-gray-300">${record.name}</td>
                            <td class="py-2 px-3 text-gray-300 font-mono text-xs">${record.content}</td>
                            <td class="py-2 px-3 text-gray-400">${record.ttl}s</td>
                            <td class="py-2 px-3">
                                <div class="flex space-x-2">
                                    <button onclick="editHostingerDNSRecord('${record.domain}', '${record.id}', ${JSON.stringify(record).replace(/"/g, '&quot;')})" 
                                            class="bg-blue-600 hover:bg-blue-700 px-2 py-1 rounded text-xs">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button onclick="deleteHostingerDNSRecord('${record.domain}', '${record.id}')" 
                                            class="bg-red-600 hover:bg-red-700 px-2 py-1 rounded text-xs">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
...
// Close Hostinger DNS manager
function closeHostingerDNSManager() {
    const modal = document.getElementById('hostingerDNSModal');
    if (modal) modal.remove();
}

// Add new DNS record to Hostinger
async function addHostingerDNSRecord(domain) {
    const apiKey = hostingerApiKey || localStorage.getItem('hostinger_api_key');
    
    const record = {
        type: document.getElementById('newHostingerRecordType').value,
        name: document.getElementById('newHostingerRecordName').value,
        content: document.getElementById('newHostingerRecordContent').value,
        ttl: parseInt(document.getElementById('newHostingerRecordTTL').value)
    };
    ...
    try {
        showNotification('Creating DNS record...', 'info');
        
        const response = await fetch(`/api/hostinger/dns-records/${domain}/create`, {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ apiKey, record })
        });
        ...
        if (data.success) {
            ...
            // Refresh the DNS records
            setTimeout(() => {
                closeHostingerDNSManager();
                manageDomainDNS(domain);
            }, 1000);
        } else {
            showNotification('Failed to create DNS record: ' + data.error, 'error');
        }
    } catch (error) {
        showNotification('Error creating DNS record: ' + error.message, 'error');
    }
}

// Delete DNS record from Hostinger
async function deleteHostingerDNSRecord(domain, recordId) {
    ...
    try {
        showNotification('Deleting DNS record...', 'info');
        
        const response = await fetch(`/api/hostinger/dns-records/${domain}/${recordId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
        ...
        if (data.success) {
            ...
            showNotification('Failed to delete DNS record: ' + data.error, 'error');
        }
    } catch (error) {
        showNotification('Error deleting DNS record: ' + error.message, 'error');
    }
}

// Sync DNS records between local system and Hostinger
async function syncDomainDNS(domain) {
    const apiKey = hostingerApiKey || localStorage.getItem('hostinger_api_key');
    ...
    try {
        showNotification(`Syncing DNS records for ${domain}...`, 'info');
        
        const response = await fetch(`/api/hostinger/sync-dns/${domain}`, {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
            ...
            body: JSON.stringify({ 
                apiKey,
                direction: 'to-hostinger' // Sync local records to Hostinger
            })
        });
        ...
        if (data.success) {
            ...
            if (data.results && data.results.length > 0) {
                ...
                if (failCount > 0) {
                    showNotification(`⚠️ ${successCount} synced, ${failCount} failed`, 'warning');
                }
            }
        } else {
            showNotification('DNS sync failed: ' + data.message, 'error');
        }
    } catch (error) {
        showNotification('Error syncing DNS: ' + error.message, 'error');
    }
}

// Add Hostinger button to DNS management interface
function addHostingerButton() {
    // Find DNS management section and add Hostinger button
    const dnsButtons = document.querySelector('#dns-tab-content-records .flex.space-x-4');
    if (dnsButtons && !document.getElementById('hostingerDNSBtn')) {
        const hostingerBtn = document.createElement('button');
        hostingerBtn.id = 'hostingerDNSBtn';
        hostingerBtn.className = 'bg-orange-600 hover:bg-orange-700 px-4 py-2 rounded-lg transition-colors flex items-center';
        hostingerBtn.innerHTML = '<i class="fas fa-server mr-2"></i>🏠 Hostinger DNS';
        hostingerBtn.onclick = showHostingerConfig;
        
        dnsButtons.appendChild(hostingerBtn);
    }
}

// Initialize Hostinger integration
document.addEventListener('DOMContentLoaded', function() {
    // Add Hostinger button after page loads
    setTimeout(addHostingerButton, 1000);
    
    // Load saved API key
    const savedApiKey = localStorage.getItem('hostinger_api_key');
    if (savedApiKey) {
        hostingerApiKey = savedApiKey;
        console.log('✅ Hostinger API key loaded from storage');
    }
});