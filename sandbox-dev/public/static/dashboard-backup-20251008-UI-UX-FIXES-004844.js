...
document.addEventListener('DOMContentLoaded', function() {
    // Force demo token for development - stronger check
    if (!token || token === 'null' || token === 'undefined' || token.trim() === '') {
        console.log('No valid auth token found, using demo token for development');
        token = 'demo';
        ...
        <div class="bg-gray-700 p-4 rounded-lg">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center space-x-4 mt-2 text-sm">
                            ...
                            <span class="flex items-center ${
                                domain.verified ? 'text-green-400' : 'text-yellow-400'
                            }">
                                <i class="fas ${domain.verified ? 'fa-shield-check' : 'fa-shield-exclamation'} mr-1 text-xs"></i>
                                ${domain.verified ? 'Doğrulandı' : 'DNS Doğrula'}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex space-x-2">
                    ...
                    ${!domain.verified ? `
                    <button onclick="showDomainVerification('${domain.id}')" 
                            class="bg-orange-600 hover:bg-orange-700 px-3 py-1 rounded text-sm" title="DNS Verification">
...
async function saveEditedDomain(domainId) {
    ...
    try {
        if (token && token !== 'null') {
            // Save to real API - only basic domain info
            const response = await fetch(`/api/domains/${domainId}`, {
                method: 'PUT',
                headers: { 
                    'Authorization': 'Bearer ' + token,
...
// Show domain verification modal
async function showDomainVerification(domainId) {
    try {
        // Get domain details
        const response = await fetch(`/api/domains`, {
            headers: { 'Authorization': 'Bearer ' + token }
        ...
        const data = await response.json();
        if (!data.success) {
            showNotification('Domain bilgisi alınamadı', 'error');
            return;
        }
        
        const domain = data.domains.find(d => d.id === domainId);
        if (!domain) {
            showNotification('Domain bulunamadı', 'error');
            return;
        }
        
        // Create verification modal
        const modal = document.createElement('div');
        modal.id = 'domainVerificationModal';
        modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
            ...
            <div class="bg-gray-800 p-6 rounded-lg max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-bold text-white">
                        <i class="fas fa-shield-exclamation mr-2 text-orange-400"></i>
                        Domain Verification - ${domain.name}
                    </h2>
                    <button onclick="closeDomainVerificationModal()" class="text-gray-400 hover:text-white">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                ...
                <div class="space-y-6">
                    <div class="bg-yellow-900 border border-yellow-600 p-4 rounded-lg">
                        ...
                        <p class="text-yellow-200 text-sm">
                            Bu domain'in trafik kontrolü için DNS doğrulaması gerekiyor. 
                            Aşağıdaki TXT kaydını domain'inizin DNS ayarlarına ekleyin.
                        </p>
                    </div>
                    
                    <div class="bg-gray-700 p-4 rounded-lg">
                        <h4 class="font-semibold text-white mb-3">DNS Record Bilgileri:</h4>
                        <div class="space-y-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-1">Record Type:</label>
                                <div class="bg-gray-900 p-2 rounded border text-green-400 font-mono">TXT</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-1">Record Name:</label>
                                <div class="bg-gray-900 p-2 rounded border text-green-400 font-mono">_trafik-verification</div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-1">Record Value:</label>
                                <div class="bg-gray-900 p-2 rounded border text-green-400 font-mono break-all">
                                    ${domain.verificationToken}
                                </div>
                                <button onclick="copyToClipboard('${domain.verificationToken}')" 
                                        class="mt-2 bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded text-sm">
                                    <i class="fas fa-copy mr-1"></i>Copy Token
                                </button>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-1">TTL:</label>
                                <div class="bg-gray-900 p-2 rounded border text-green-400 font-mono">300 (5 minutes)</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-900 border border-blue-600 p-4 rounded-lg">
                        ...
                        <ol class="text-blue-200 text-sm space-y-1 ml-4 list-decimal">
                            ...
                            <li>Websites → Manage → DNS Zone Editor</li>
                            <li>"Add New Record" butonuna tıklayın</li>
                            <li>Type: TXT, Name: _trafik-verification, Value: ${domain.verificationToken}</li>
                            <li>Save Changes</li>
                            <li>5-10 dakika bekleyin (DNS propagation)</li>
                            <li>Aşağıdaki "Verify Now" butonuna tıklayın</li>
                        </ol>
                    </div>
                    
                    <div class="bg-gray-700 p-4 rounded-lg">
                        <h4 class="font-semibold text-white mb-2">Verification Status:</h4>
                        <div id="verificationStatus" class="text-yellow-400">
                            <i class="fas fa-clock mr-2"></i>
                            DNS kaydı eklendikten sonra doğrulama yapabilirsiniz
                        </div>
                        <div class="mt-3 flex space-x-3">
                            <button onclick="verifyDomainNow('${domainId}')" 
                                    class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded font-semibold">
                                <i class="fas fa-check mr-2"></i>Verify Now
                            </button>
                            <button onclick="closeDomainVerificationModal()" 
                                    class="bg-gray-600 hover:bg-gray-700 px-4 py-2 rounded">
        ...
        showNotification('Verification modal hatası: ' + error.message, 'error');
    }
}

// Close domain verification modal
function closeDomainVerificationModal() {
    const modal = document.getElementById('domainVerificationModal');
    if (modal) {
        document.body.removeChild(modal);
    }
}
...
// Verify domain now
async function verifyDomainNow(domainId) {
    try {
        showNotification('Domain doğrulanıyor...', 'info');
        ...
        if (data.success) {
            ...
            `;
            showNotification('Domain başarıyla doğrulandı! 🎉', 'success');
            
            // Refresh domains and close modal after delay
            setTimeout(() => {
                loadDomains();
                closeDomainVerificationModal();
            }, 2000);
        } else {
            statusDiv.innerHTML = `
                ...
                <div class="text-sm text-gray-400 mt-2">
                    Attempt ${data.domain?.verificationAttempts || 0}
                </div>
            `;
            showNotification('Doğrulama başarısız: ' + data.message, 'error');
        }
    } catch (error) {
        showNotification('Verification hatası: ' + error.message, 'error');
    }
}
