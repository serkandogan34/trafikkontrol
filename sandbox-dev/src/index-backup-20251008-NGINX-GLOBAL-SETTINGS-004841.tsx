...
# Rate limiting zones
limit_req_zone $binary_remote_addr zone=general:10m rate=${globalSettings.rateLimit || 10}r/s;
limit_req_zone $binary_remote_addr zone=bots:10m rate=${globalSettings.botRateLimit || 1}r/s;

# GeoIP configuration ${globalSettings.enableGeoIP ? '(enabled)' : '(disabled)'}
${globalSettings.enableGeoIP ? 'geoip_country /usr/share/GeoIP/GeoIP.dat;' : '# geoip_country /usr/share/GeoIP/GeoIP.dat;'}
...
server {
  ...
  config += `
        ...
        local security_settings = {
            advanced_bot_protection = ${globalSettings.advancedBotProtection ? 'true' : 'false'},
            ddos_protection = ${globalSettings.ddosProtection ? 'true' : 'false'},
            facebook_referrer_detection = ${globalSettings.facebookReferrerDetection ? 'true' : 'false'},
            block_suspicious_traffic = ${globalSettings.blockSuspiciousTraffic ? 'true' : 'false'}
        }
        ...
        -- Enhanced security checks based on global settings
        if security_settings.advanced_bot_protection then
            -- Additional sophisticated bot detection
            if string.find(lower_ua, "automation") or string.find(lower_ua, "script") then
                bot_score = bot_score + 7
            end
            
            -- Check for suspicious request patterns
            if security_settings.ddos_protection then
                -- Simple rate limiting check (more sophisticated in real implementation)
                local request_count = ngx.shared.rate_limiting:get(remote_addr) or 0
                if request_count > 100 then
                    return ngx.exit(429)  -- Too Many Requests
                end
                ngx.shared.rate_limiting:set(remote_addr, request_count + 1, 60)
            end
        end
        ...
        if referer ~= "" then
            ...
            if string.find(lower_ref, "facebook") or string.find(lower_ref, "fb%.") then
                ...
                -- Facebook referrer detection enforcement
                if security_settings.facebook_referrer_detection and is_bot then
                    -- More lenient for Facebook referrers
        ...
        -- Block suspicious traffic if enabled
        if security_settings.block_suspicious_traffic and bot_score > 15 and not social_referrer then
        ...
        -- Log traffic for analytics (if enabled)
        if ${globalSettings.enableTrafficAnalytics ? 'true' : 'false'} then
...
app.post('/api/domains', requireAuth, async (c) => {
  ...
  const domain = {
    ...
    verified: false, // Domain sahipliği doğrulandı mı?
    verificationToken: `trafik-${Date.now()}-${Math.random().toString(36).substring(7)}`, // Unique token
    verificationMethod: 'TXT', // TXT record ile doğrulama
    verifiedAt: null, // Doğrulama tarihi
    verificationAttempts: 0, // Doğrulama denemesi sayısı
    lastVerificationCheck: null // Son doğrulama kontrolü
  }
...
app.post('/api/domains/:id/verify', requireAuth, async (c) => {
  const id = c.req.param('id')
  ...
  try {
    // DNS TXT record kontrolü
    const txtRecords = await checkDNSTxtRecord(domain.name, domain.verificationToken)
    ...
    if (txtRecords.found) {
      ...
      return c.json({ 
        success: true, 
        message: 'Domain başarıyla doğrulandı! Artık trafik kontrolü aktif.',
      ...
      return c.json({ 
        success: false, 
        message: 'Domain doğrulaması başarısız. TXT kaydının doğru eklendiğinden emin olun.',
        domain: updatedDomain,
        instructions: {
          recordType: 'TXT',
          recordName: '_trafik-verification',
...
// DNS TXT Record Check Function
async function checkDNSTxtRecord(domain, expectedToken) {
  try {
    // Simulated DNS lookup - gerçekte DNS API kullanılacak
    // Bu fonksiyon gerçek DNS sorgusu yapacak
    
    const recordName = `_trafik-verification.${domain}`
    
    // Simulated DNS check result
    // Gerçekte: const response = await dns.resolveTxt(recordName)
    const simulatedFound = Math.random() > 0.5 // Test amaçlı
    
    if (simulatedFound) {
      return {
        found: true,
        record: `_trafik-verification.${domain}`,
        value: expectedToken,
        ttl: 300,
        provider: 'detected'
      }
...
app.post('/api/nginx/generate-config', requireAuth, async (c) => {
  try {
    ...
    const config = generateAdvancedNginxConfig({
      ...
      globalSettings: { ...nginxGlobalSettings, ...globalSettings }
    })
...
app.get('/api/nginx/download-config', requireAuth, async (c) => {
  try {
    ...
    const config = generateAdvancedNginxConfig({
      ...
      globalSettings: nginxGlobalSettings
    })
...
app.get('/api/nginx/global-settings', requireAuth, (c) => {
  return c.json({
    success: true,
...
app.put('/api/nginx/global-settings', requireAuth, async (c) => {
  try {
    const updates = await c.req.json()
    nginxGlobalSettings = { ...nginxGlobalSettings, ...updates }
...
// ====================================================================
// DOMAIN TRAFFIC MANAGEMENT - CATCH-ALL ROUTE (BORU HATTI)
// ====================================================================

// Domain-based traffic handler - IP Havuzuna Dolum Sistemi
app.all('*', async (c, next) => {
  const hostname = c.req.header('host')
  const path = c.req.path
  
  // Admin Panel Korunuyor (mevcut sistem aynen devam eder)
  if (hostname?.includes('localhost') || 
    ...
    // Mevcut admin route'larına geç
    await next()
    return
  }
  
  // Gerçek Domain Trafiği - BORU HATTI AKTİF!
  const clientIP = trackIPCall(c, path) // Mevcut fonksiyonu kullan
  if (!clientIP) {
    return c.text('IP tracking failed', 500)
  }
  
  // Domain konfigürasyonu bul
  const domainConfig = Array.from(domains.values()).find(d => d.name === hostname)
  ...
  if (!domainConfig.verified) {
    return c.text('Domain verification required', 403)
  }
  
  // IP havuzundan karar al
  const ipData = ipPoolManager.pool.get(clientIP)
  
  if (!ipData) {
    // İlk ziyaret - cautious approach
    return c.text('Processing your request...', 202)
  }
  
  // Traffic decision engine
  const decision = makeTrafficDecision(ipData, domainConfig)
  
  switch (decision.action) {
    case 'allow':
      // TODO: Backend proxy eklenecek
      return c.text(`Welcome! (IP: ${clientIP}, Classification: ${ipData.classification})`, 200)
      
    case 'block':
      return c.text('Access Denied', 403)
      
    case 'monitor':
      return c.text('Under Review', 202)
...
// Traffic Decision Engine
function makeTrafficDecision(ipData, domainConfig) {
  // Manuel statüs önceliklidir
  if (ipData.manualStatus === 'whitelisted') {
    return { action: 'allow', reason: 'Manually whitelisted' }
  }
  
  if (ipData.manualStatus === 'blacklisted') {
    return { action: 'block', reason: 'Manually blacklisted' }
  }
  
  // Otomatik sınıflandırma bazlı kararlar
  switch (ipData.classification) {
    case 'whitelisted':
    case 'normal':
    case 'frequent':
      return { action: 'allow', reason: `Classification: ${ipData.classification}` }
      
    case 'blocked':
    case 'high_risk':
      return { action: 'block', reason: `High risk: ${ipData.classification}` }
      
    case 'suspicious':
    case 'analysis_required':
      return { action: 'monitor', reason: `Under analysis: ${ipData.classification}` }
      
    case 'new':
    default:
      return { action: 'monitor', reason: 'New visitor - monitoring' }
  }
}
...
http://localhost:3000"></textarea>
        ...
        <!-- Feature Systems - Inline Implementation -->
        <script>
          // Complete mock feature managers implementation
          window.reactFeatures = {
            enabled: true,
            isEnabled: () => true,
            init: () => console.log('React features initialized'),
            status: () => ({ reactFeatures: true }),
            toggle: () => console.log('React features toggled'),
            start: () => console.log('React features started'),
            stop: () => console.log('React features stopped'),
            getComponents: () => ([]),
            loadComponent: (name) => console.log('Loading component: ' + name)
          };
          
          window.wsManager = {
            enabled: true,
            isEnabled: () => true,
            connect: () => console.log('WebSocket manager initialized'),
            disconnect: () => console.log('WebSocket manager disconnected'),
            status: () => ({ wsManager: true }),
            isConnected: () => false,
          ...
          window.aiTracker = {
            enabled: true,
            isEnabled: () => true,
            init: () => console.log('AI tracker initialized'),
            track: (event) => console.log('Tracking event:', event),
            status: () => ({ aiTracker: true }),
            getStats: () => ({ events: 0, users: 0 }),
            start: () => console.log('AI tracker started'),
            stop: () => console.log('AI tracker stopped')
          };
          
          // Initialize features immediately
          setTimeout(() => {
            if (window.reactFeatures) window.reactFeatures.init();
            if (window.wsManager) window.wsManager.connect();
            if (window.aiTracker) window.aiTracker.init();
          }, 100);
        </script>
