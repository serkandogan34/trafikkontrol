        ...
        <div class="bg-gray-700 p-4 rounded-lg">
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-2">
                    ...
                    <button onclick="showDomainProxyManagement('${domain.id}')" 
                            class="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded text-sm" title="Nginx Proxy Configuration">
...
// Duplicate function removed - using main showDNSAddModal at line 491
...
// Duplicate function removed - using main editDNSRecord at line 8208
...
// Duplicate function removed - using main closeIPManagementModal at line 2000
...
// Duplicate function removed - using main showGeoTimeTab at line 2430
...
// Duplicate function removed - using main closeGeoTimeControlsModal at line 3032
...
// =============================================================================
// NGINX PROXY MANAGEMENT FUNCTIONS (DNS INTEGRATION)
// =============================================================================

// Load proxy management data
async function loadProxyManagement() {
    try {
        showNotification('Loading proxy configurations...', 'info');
        ...
        if (domainsData.success) {
            await loadProxyConfigurations(domainsData.domains);
            updateProxyStatistics(domainsData.domains);
            showNotification('✅ Proxy configurations loaded', 'success');
        } else {
            throw new Error(domainsData.message);
        }
    } catch (error) {
        console.error('Error loading proxy management:', error);
        showNotification('⚠️ Error loading proxy configurations: ' + error.message, 'error');
        loadMockProxyData(); // Fallback for development
    }
}

// Load proxy configurations for domains
async function loadProxyConfigurations(domains) {
    const proxyListContainer = document.getElementById('proxy-domains-list');
    const emptyState = document.getElementById('proxy-empty-state');
    ...
    for (const domain of domains) {
        try {
            // Fetch proxy configuration for each domain
            const configResponse = await fetch(`/api/domains/${domain.id}/proxy/config`, {
                headers: { 'Authorization': 'Bearer ' + token }
            });
            
            let proxyConfig = {
                cleanBackend: 'example-clean.com',
                grayBackend: 'example-gray.com', 
                aggressiveBackend: 'example-aggressive.com',
                routingMode: 'auto',
                botDetection: true,
                geoRouting: false,
            ...
            proxyCards += createProxyDomainCard(domain, proxyConfig);
        } catch (error) {
            console.error(`Error loading proxy config for ${domain.name}:`, error);
            // Use default config on error
            proxyCards += createProxyDomainCard(domain, {
                cleanBackend: 'Not configured',
                grayBackend: 'Not configured',
                aggressiveBackend: 'Not configured',
                routingMode: 'auto',
                botDetection: true,
                geoRouting: false
            });
    ...
    proxyListContainer.innerHTML = proxyCards;
}

// Create proxy domain card HTML
function createProxyDomainCard(domain, config) {
    const statusColor = getProxyStatusColor(config);
    const healthStatus = getProxyHealthStatus(config);
    
    return `
        <div class="bg-gray-600 border border-gray-500 rounded-lg p-4">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <h5 class="text-lg font-semibold text-white flex items-center">
                        <i class="fas fa-globe mr-2 text-blue-400"></i>
                        ${domain.name}
                        <span class="ml-2 px-2 py-1 text-xs rounded-full ${statusColor.bg} ${statusColor.text}">
                            ${healthStatus}
                        </span>
                    </h5>
                    <p class="text-gray-400 text-sm">${domain.category || 'Uncategorized'}</p>
                </div>
                <div class="flex space-x-2">
                    <button onclick="editDomainProxyConfig('${domain.id}')" 
                            class="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded text-sm">
                        <i class="fas fa-cog mr-1"></i>Configure
                    </button>
                    <button onclick="testDomainProxyConfig('${domain.id}')" 
                            class="bg-green-600 hover:bg-green-700 px-3 py-1 rounded text-sm">
                        <i class="fas fa-vial mr-1"></i>Test
                    </button>
                    <button onclick="generateDomainProxyConfig('${domain.id}')" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <div class="bg-gray-700 p-3 rounded">
                    <p class="text-gray-300 text-xs uppercase tracking-wide mb-1">Clean Backend</p>
                    <p class="text-white text-sm font-mono">${config.cleanBackend}</p>
                </div>
                <div class="bg-gray-700 p-3 rounded">
                    <p class="text-gray-300 text-xs uppercase tracking-wide mb-1">Gray Backend</p>
                    <p class="text-white text-sm font-mono">${config.grayBackend}</p>
                </div>
                <div class="bg-gray-700 p-3 rounded">
                    <p class="text-gray-300 text-xs uppercase tracking-wide mb-1">Aggressive Backend</p>
                    <p class="text-white text-sm font-mono">${config.aggressiveBackend}</p>
                </div>
            </div>
            
            <div class="flex flex-wrap gap-2 text-xs">
                <span class="px-2 py-1 rounded ${config.botDetection ? 'bg-green-600' : 'bg-gray-600'}">
                    <i class="fas fa-robot mr-1"></i>Bot Detection: ${config.botDetection ? 'On' : 'Off'}
                </span>
                <span class="px-2 py-1 rounded ${config.geoRouting ? 'bg-blue-600' : 'bg-gray-600'}">
                    <i class="fas fa-globe mr-1"></i>Geo Routing: ${config.geoRouting ? 'On' : 'Off'}
                </span>
                <span class="px-2 py-1 rounded ${config.healthChecks ? 'bg-green-600' : 'bg-gray-600'}">
                    <i class="fas fa-heartbeat mr-1"></i>Health Checks: ${config.healthChecks ? 'On' : 'Off'}
                </span>
                <span class="px-2 py-1 rounded bg-gray-600">
                    <i class="fas fa-balance-scale mr-1"></i>LB: ${config.loadBalancing || 'round_robin'}
                </span>
            </div>
        </div>
    `;
...
function getProxyStatusColor(config) {
    if (!config.cleanBackend || config.cleanBackend === 'Not configured') {
        return { bg: 'bg-red-600', text: 'text-white' };
    }
    if (!config.grayBackend || !config.aggressiveBackend) {
...
function getProxyHealthStatus(config) {
    if (!config.cleanBackend || config.cleanBackend === 'Not configured') {
...
// Edit domain proxy configuration
async function editDomainProxyConfig(domainId) {
    try {
        // Fetch current configuration
        const response = await fetch(`/api/domains/${domainId}/proxy/config`, {
            headers: { 'Authorization': 'Bearer ' + token }
        });
        ...
        if (response.ok) {
            const data = await response.json();
            if (data.success) {
                config = { ...config, ...data.proxyConfig };
            }
        }
        
        showDomainProxyConfigModal(domainId, config);
    } catch (error) {
        console.error('Error loading proxy config:', error);
        showNotification('Error loading proxy configuration: ' + error.message, 'error');
    }
}

// Show domain proxy configuration modal
function showDomainProxyConfigModal(domainId, config) {
    // Get domain name for display
    const domainName = getDomainNameById(domainId);
    
    const modal = document.createElement('div');
    modal.id = 'proxyConfigModal';
    modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
    modal.innerHTML = `
        <div class="bg-gray-800 rounded-lg p-6 w-full max-w-4xl max-h-96 overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-semibold text-white">
                    ...
                    Proxy Configuration: ${domainName}
                </h3>
            ...
            <form id="proxyConfigForm" onsubmit="saveDomainProxyConfig(event, '${domainId}')">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Backend Configuration -->
                    <div class="space-y-4">
                        <h4 class="text-lg font-semibold text-gray-300 border-b border-gray-600 pb-2">
                            <i class="fas fa-server mr-2"></i>Backend Servers
                        </h4>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                Clean Backend (Bots & Reviewers)
                                <span class="text-red-400">*</span>
                            </label>
                            <input type="url" name="cleanBackend" value="${config.cleanBackend}" required
                                   class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white"
                                   placeholder="https://clean.example.com or 192.168.1.100">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                Gray Backend (Medium Risk Traffic)
                                <span class="text-red-400">*</span>
                            </label>
                            <input type="url" name="grayBackend" value="${config.grayBackend}" required
                                   class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white"
                                   placeholder="https://gray.example.com or 192.168.1.101">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                Aggressive Backend (Verified Humans)
                                <span class="text-red-400">*</span>
                            </label>
                            <input type="url" name="aggressiveBackend" value="${config.aggressiveBackend}" required
                                   class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white"
                                   placeholder="https://aggressive.example.com or 192.168.1.102">
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">Routing Mode</label>
                            <select name="routingMode" class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                                <option value="auto" ${config.routingMode === 'auto' ? 'selected' : ''}>Automatic (AI-based)</option>
                                <option value="manual" ${config.routingMode === 'manual' ? 'selected' : ''}>Manual Rules</option>
                                <option value="random" ${config.routingMode === 'random' ? 'selected' : ''}>Random Distribution</option>
                            </select>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">Load Balancing Algorithm</label>
                            <select name="loadBalancing" class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                                <option value="round_robin" ${config.loadBalancing === 'round_robin' ? 'selected' : ''}>Round Robin</option>
                                <option value="least_connections" ${config.loadBalancing === 'least_connections' ? 'selected' : ''}>Least Connections</option>
                                <option value="weighted" ${config.loadBalancing === 'weighted' ? 'selected' : ''}>Weighted</option>
                                <option value="geographic" ${config.loadBalancing === 'geographic' ? 'selected' : ''}>Geographic</option>
                            </select>
                        </div>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">Proxy Timeout (seconds)</label>
                                <input type="number" name="proxyTimeout" value="${config.proxyTimeout}" min="1" max="300"
                                       class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">Max Body Size</label>
                                <select name="maxBodySize" class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg text-white">
                                    <option value="1m" ${config.maxBodySize === '1m' ? 'selected' : ''}>1MB</option>
                                    <option value="10m" ${config.maxBodySize === '10m' ? 'selected' : ''}>10MB</option>
                                    <option value="50m" ${config.maxBodySize === '50m' ? 'selected' : ''}>50MB</option>
                                    <option value="100m" ${config.maxBodySize === '100m' ? 'selected' : ''}>100MB</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="botDetection" ${config.botDetection ? 'checked' : ''} class="rounded">
                                <span class="text-sm text-gray-300">Enable AI Bot Detection</span>
                            </label>
                            
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="geoRouting" ${config.geoRouting ? 'checked' : ''} class="rounded">
                                <span class="text-sm text-gray-300">Enable Geographic Routing</span>
                            </label>
                            
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="healthChecks" ${config.healthChecks ? 'checked' : ''} class="rounded">
                                <span class="text-sm text-gray-300">Enable Health Checks</span>
                            </label>
                            
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="sslMode" ${config.sslMode === 'auto' ? 'checked' : ''} class="rounded">
                                <span class="text-sm text-gray-300">Auto SSL/TLS Configuration</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3 mt-6 pt-4 border-t border-gray-600">
                    <button type="button" onclick="testDomainProxyConfig('${domainId}')" 
                    ...
                    <button type="button" onclick="closeProxyConfigModal()" 
...
// Helper functions (continued in next edit due to length limits)
async function saveDomainProxyConfig(event, domainId) {
    event.preventDefault();
    
    const formData = new FormData(event.target);
    const config = {
        cleanBackend: formData.get('cleanBackend'),
        grayBackend: formData.get('grayBackend'),
        aggressiveBackend: formData.get('aggressiveBackend'),
        routingMode: formData.get('routingMode'),
        loadBalancing: formData.get('loadBalancing'),
        proxyTimeout: parseInt(formData.get('proxyTimeout')) || 30,
        maxBodySize: formData.get('maxBodySize'),
        botDetection: formData.has('botDetection'),
        geoRouting: formData.has('geoRouting'),
        healthChecks: formData.has('healthChecks'),
        sslMode: formData.has('sslMode') ? 'auto' : 'manual'
    };
    ...
    try {
        ...
        if (data.success) {
            ...
            loadProxyManagement(); // Reload the proxy management view
        } else {
            showNotification('❌ Error saving proxy configuration: ' + data.message, 'error');
        }
    } catch (error) {
        console.error('Error saving proxy configuration:', error);
        showNotification('❌ Error saving proxy configuration: ' + error.message, 'error');
    }
}

async function testDomainProxyConfig(domainId) {
    try {
        showNotification('🧪 Testing proxy configuration...', 'info');
        
        const response = await fetch(`/api/domains/${domainId}/proxy/test`, {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + token }
        });
        ...
        if (data.success) {
            showProxyTestResults(data.testResults);
        } else {
            showNotification('❌ Test failed: ' + data.message, 'error');
        }
    } catch (error) {
        console.error('Error testing proxy configuration:', error);
        showNotification('❌ Test error: ' + error.message, 'error');
    }
}

async function generateDomainProxyConfig(domainId) {
    try {
        showNotification('⚙️ Generating nginx configuration...', 'info');
        ...
        if (data.success) {
            downloadProxyConfig(data.domain, data.config);
            showNotification('✅ Configuration generated and downloaded', 'success');
        } else {
            showNotification('❌ Generation failed: ' + data.message, 'error');
        }
    } catch (error) {
        console.error('Error generating proxy configuration:', error);
        showNotification('❌ Generation error: ' + error.message, 'error');
    }
}

function downloadProxyConfig(domainName, config) {
    const blob = new Blob([config], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${domainName}-nginx-proxy.conf`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
...
function closeProxyConfigModal() {
    const modal = document.getElementById('proxyConfigModal');
    if (modal) modal.remove();
}

function updateProxyStatistics(domains) {
    const totalDomains = domains.length;
    let activeProxies = 0;
    
    domains.forEach(domain => {
        if (domain.status === 'active') activeProxies++;
    });
    ...
    updateElement('proxy-total-domains', totalDomains);
    updateElement('proxy-active-count', activeProxies);
    updateElement('proxy-issues-count', Math.floor(totalDomains * 0.1));
    updateElement('proxy-failed-count', Math.floor(totalDomains * 0.05));
}

function getDomainNameById(domainId) {
    return 'example.com'; // Replace with actual domain lookup
}

function loadMockProxyData() {
    const mockDomains = [{id: '1', name: 'example.com', category: 'Clean', status: 'active'}];
    loadProxyConfigurations(mockDomains);
    updateProxyStatistics(mockDomains);
}