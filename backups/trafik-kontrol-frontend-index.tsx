...
async function performBotDetection(trafficData: any): Promise<boolean> {
...
// IP Risk Assessment
async function assessIPRisk(ip: string, domain: string): Promise<string> {
...
// Decision Engine
function decideProxyAction(isBot: boolean, riskLevel: string, domain: string) {
...
app.post('/api/domains', requireAuth, async (c) => {
  const { name, originalBackend, description } = await c.req.json()
  ...
  if (!originalBackend) {
    return c.json({ success: false, message: 'Original Backend IP gerekli' }, 400)
  }
  
  // Clean domain name
  const cleanDomain = name.replace(/^https?:\/\//, '').replace(/\/$/, '')
  
  // Validate and clean backend IP
  const cleanBackend = originalBackend.replace(/^https?:\/\//, '')
...
const HOSTINGER_CONFIG = {
  ...
  endpoints: {
    domains: '/domains',
    dnsRecords: '/domains/{domain}/dns-records',
    bulkDnsRecords: '/domains/{domain}/dns-records/bulk'
  }
}

// Hostinger API Helper Functions
async function makeHostingerAPIRequest(endpoint, options = {}) {
  const apiKey = process.env.HOSTINGER_API_KEY || options.apiKey
  
  if (!apiKey) {
    throw new Error('Hostinger API key not configured')
  }
  
  const url = `${HOSTINGER_CONFIG.baseUrl}${endpoint}`
  const response = await fetch(url, {
    ...options,
    headers: {
...
// Get Hostinger domains list
async function getHostingerDomains(apiKey) {
  try {
    const data = await makeHostingerAPIRequest(HOSTINGER_CONFIG.endpoints.domains, {
      method: 'GET',
      apiKey
    })
...
// Get DNS records for a domain from Hostinger
async function getHostingerDNSRecords(domain, apiKey) {
  try {
    const endpoint = HOSTINGER_CONFIG.endpoints.dnsRecords.replace('{domain}', domain)
    const data = await makeHostingerAPIRequest(endpoint, {
      method: 'GET',
      apiKey
    })
...
// Create DNS record in Hostinger
async function createHostingerDNSRecord(domain, record, apiKey) {
  try {
    const endpoint = HOSTINGER_CONFIG.endpoints.dnsRecords.replace('{domain}', domain)
    const data = await makeHostingerAPIRequest(endpoint, {
      method: 'POST',
...
async function updateHostingerDNSRecord(domain, recordId, record, apiKey) {
  try {
    const endpoint = `${HOSTINGER_CONFIG.endpoints.dnsRecords.replace('{domain}', domain)}/${recordId}`
    const data = await makeHostingerAPIRequest(endpoint, {
      method: 'PUT',
      apiKey,
      body: JSON.stringify({
...
async function deleteHostingerDNSRecord(domain, recordId, apiKey) {
  try {
    const endpoint = `${HOSTINGER_CONFIG.endpoints.dnsRecords.replace('{domain}', domain)}/${recordId}`
    await makeHostingerAPIRequest(endpoint, {
      method: 'DELETE',
      apiKey
    })
...
// =============================================================================
// HOSTINGER DNS API ENDPOINTS
// =============================================================================

// Test Hostinger API connection
app.post('/api/hostinger/test-connection', requireAuth, async (c) => {
  try {
    const { apiKey } = await c.req.json()
    
    if (!apiKey) {
      return c.json({ success: false, message: 'API key is required' })
    }
    
    const result = await getHostingerDomains(apiKey)
    
    if (result.success) {
      return c.json({ 
        success: true, 
        message: 'Hostinger API connection successful',
...
// Get Hostinger domains
app.post('/api/hostinger/domains', requireAuth, async (c) => {
  try {
    const { apiKey } = await c.req.json()
...
// Get DNS records for a domain from Hostinger
app.post('/api/hostinger/dns-records/:domain', requireAuth, async (c) => {
  try {
    const domain = c.req.param('domain')
    const { apiKey } = await c.req.json()
    ...
    const result = await getHostingerDNSRecords(domain, apiKey)
    return c.json(result)
  } catch (error) {
    return c.json({ 
      success: false, 
...
// Create DNS record in Hostinger
app.post('/api/hostinger/dns-records/:domain/create', requireAuth, async (c) => {
  try {
    const domain = c.req.param('domain')
    const { apiKey, record } = await c.req.json()
    
    if (!apiKey || !record) {
      return c.json({ success: false, message: 'API key and record data are required' })
    }
    
    const result = await createHostingerDNSRecord(domain, record, apiKey)
    
    if (result.success) {
      console.log(`DNS record created in Hostinger for ${domain}:`, record)
    }
...
// Update DNS record in Hostinger  
app.put('/api/hostinger/dns-records/:domain/:recordId', requireAuth, async (c) => {
  try {
    const domain = c.req.param('domain')
    ...
    const result = await updateHostingerDNSRecord(domain, recordId, record, apiKey)
    
    if (result.success) {
      console.log(`DNS record updated in Hostinger for ${domain}:`, record)
    }
...
// Delete DNS record from Hostinger
app.delete('/api/hostinger/dns-records/:domain/:recordId', requireAuth, async (c) => {
  try {
    const domain = c.req.param('domain')
    const recordId = c.req.param('recordId')
    const { apiKey } = await c.req.json()
    ...
    const result = await deleteHostingerDNSRecord(domain, recordId, apiKey)
    
    if (result.success) {
      console.log(`DNS record deleted from Hostinger for ${domain}: ${recordId}`)
    }
...
// Sync DNS records between local and Hostinger
app.post('/api/hostinger/sync-dns/:domain', requireAuth, async (c) => {
  try {
    const domain = c.req.param('domain')
    const { apiKey, direction = 'to-hostinger' } = await c.req.json()
    
    if (!apiKey) {
      return c.json({ success: false, message: 'API key is required' })
    }
    
    // Get local DNS records for the domain
    const localRecords = Array.from(dnsRecords.values()).filter(record => 
      record.domain === domain
    )
    
    let syncResults = []
    
    if (direction === 'to-hostinger') {
      // Sync local records to Hostinger
      for (const localRecord of localRecords) {
        const hostingerRecord = {
          type: localRecord.type,
          name: localRecord.name,
          content: localRecord.value,
          ttl: localRecord.ttl || 3600
        }
        
        const result = await createHostingerDNSRecord(domain, hostingerRecord, apiKey)
        syncResults.push({
          localRecord: localRecord.name,
    ...
    return c.json({ 
      success: true, 
      message: `Synced ${syncResults.filter(r => r.success).length} of ${syncResults.length} records`,
      results: syncResults
    })
  } catch (error) {
    return c.json({ 
        ...
        <div id="addDomainModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div class="bg-gray-800 p-6 rounded-lg w-96">
                ...
                <form id="addDomainForm">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Original Backend IP</label>
                        <input type="text" id="originalBackend" required
                               class="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg focus:border-blue-400 focus:outline-none"
                               placeholder="207.180.204.60:80">
                        <p class="text-xs text-gray-400 mt-1">Mevcut web sitenizin hosting IP:Port adresi</p>
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Açıklama</label>
                        <input type="text" id="domainDescription"
                               class="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg focus:border-blue-400 focus:outline-none"
                               placeholder="Mevcut Hostinger web sitesi">
                        <p class="text-xs text-gray-400 mt-1">Domain hakkında kısa açıklama (opsiyonel)</p>
                    </div>
